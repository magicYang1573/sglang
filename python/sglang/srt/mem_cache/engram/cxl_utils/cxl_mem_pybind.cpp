...
void tensor_to_cxl(torch::Tensor src, std::size_t offset) {
  ...
  if (t.is_cuda()) {
    ...
    // throw_if_false(vram2cxl_async(t.data_ptr(), bytes, offset),
    // "vram2cxl_async");
  } else {
...
torch::Tensor cxl_to_tensor_noflush(torch::Tensor dst, std::size_t offset) {
  ...
  if (t.is_cuda()) {
    throw_if_false(cxl2vram_noflush(t.data_ptr(), bytes, offset),
                   "cxl2vram_noflush");
  } else {
    throw_if_false(cxl2dram_noflush(t.data_ptr(), bytes, offset),
                   "cxl2dram_noflush");
  }
...
cxl_to_tensor_noflush_parallel(std::vector<torch::Tensor> dst_list,
  ...
  if (offsets.scalar_type() != at::kLong) {
    throw std::runtime_error(
        "cxl_to_tensor_noflush_parallel: offsets must be int64 CPU tensor");
  }
  const std::size_t count = static_cast<std::size_t>(offsets.numel());
  if (count != dst_list.size()) {
    throw std::runtime_error(
        "cxl_to_tensor_noflush_parallel: offsets size must match tensors size");
  }
  ...
  if (any_cuda && any_cpu) {
    throw std::runtime_error("cxl_to_tensor_noflush_parallel: mixed CPU/CUDA "
  ...
  if (any_cuda) {
    for (std::size_t i = 0; i < count; ++i) {
      throw_if_false(
          cxl2vram_noflush(dst_ptrs[i], bytes_list[i], offsets_size_t[i]),
          "cxl2vram_noflush");
    }
  } else {
    auto start_time = std::chrono::high_resolution_clock::now();
    throw_if_false(cxl2dram_noflush_parallel(dst_ptrs.data(), bytes_list.data(),
                                             offsets_size_t.data(),
                                             dst_ptrs.size(), 64),
...
torch::Tensor cxl_to_tensor_noflush_parallel_contiguous_dst(
    at::Tensor dst, std::size_t bytes_per_segment, at::Tensor offset_list) {
  auto t = ensure_contiguous(dst);
  auto offsets = ensure_contiguous(offset_list);
  if (offsets.is_cuda()) {
    offsets = offsets.to(at::kCPU);
  ...
  if (offsets.scalar_type() != at::kLong) {
    throw std::runtime_error("cxl_to_tensor_noflush_parallel_contiguous_dst: "
                             "offsets must be int64 CPU tensor");
  }
  const std::size_t count = static_cast<std::size_t>(offsets.numel());
  const std::size_t total_bytes = bytes_per_segment * count;
  if (total_bytes > static_cast<std::size_t>(t.nbytes())) {
    throw std::runtime_error(
        "cxl_to_tensor_noflush_parallel_contiguous_dst: dst too small");
  }
  ...
  if (t.is_cuda()) {
    throw_if_false(
        cxl2vram_noflush_parallel_contiguous_dst(
            t.data_ptr(), bytes_per_segment, offsets_size_t.data(), count),
        "cxl2vram_noflush_parallel_contiguous_dst");
  } else {
    throw_if_false(
...
PYBIND11_MODULE(cxl_mem_ext, m) {
  ...
  m.def("tensor_to_cxl", &tensor_to_cxl, pybind11::arg("tensor"),
        pybind11::arg("offset") = 0,
        "Copy a tensor (CPU or CUDA) into the mapped CXL window at the given "
        "offset.");

  m.def("cxl_to_tensor", &cxl_to_tensor, pybind11::arg("tensor"),
        pybind11::arg("offset") = 0,
        "Fill a tensor (CPU or CUDA) from the mapped CXL window at the given "
        "offset.");

  m.def("cxl_to_tensor_noflush", &cxl_to_tensor_noflush,
        pybind11::arg("tensor"), pybind11::arg("offset") = 0,
        "Fill a tensor (CPU or CUDA) from the mapped CXL window at the given "
        "offset without flushing.");

  m.def("cxl_to_tensor_noflush_parallel", &cxl_to_tensor_noflush_parallel,
        pybind11::arg("tensors"), pybind11::arg("offsets"),
        "Fill tensors (CPU or CUDA) from the mapped CXL window at given "
        "offsets without flushing."
        " CPU path uses OpenMP with 64 threads; CUDA path is serial.");

  m.def("cxl_to_tensor_noflush_parallel_contiguous_dst",
        &cxl_to_tensor_noflush_parallel_contiguous_dst, pybind11::arg("tensor"),
        pybind11::arg("bytes_per_segment"), pybind11::arg("offsets"),
        "Fill a contiguous tensor from the mapped CXL window at given offsets "
        "with fixed segment size.");

  m.def("cxl_barrier_tp", &cxl_barrier_tp, pybind11::arg("token"),
        pybind11::arg("control_offset"), pybind11::arg("rank"),
        pybind11::arg("num_ranks"), "CXL barrier for tensor parallelism.");
}
