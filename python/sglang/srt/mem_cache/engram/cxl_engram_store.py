...
"""CXL shared-memory backed Engram embedding store.

Uses a CXL (Compute Express Link) DAX device for shared-memory embedding
storage.  A custom C++/CUDA extension (``cxl_mem_ext``) provides low-level
memory-mapped I/O.  This backend is designed for ultra-low-latency access
on hardware that exposes CXL memory as a DAX device.
"""
...
class CxlEngramStoreConfig:
    ...
    @staticmethod
    def from_env(
        base: Optional["CxlEngramStoreConfig"] = None,
    ) -> "CxlEngramStoreConfig":
        base = base or CxlEngramStoreConfig()
        register_cuda_default = base.register_cuda or torch.cuda.is_available()
        return CxlEngramStoreConfig(
...
_GLOBAL_EXT = None
_GLOBAL_CONFIG: Optional[CxlEngramStoreConfig] = None
_GLOBAL_INIT = False
...
def _load_cxl_extension(config: CxlEngramStoreConfig):
    global _GLOBAL_EXT
    ...
    try:
        ...
        root = Path(__file__).resolve().parent / "cxl_utils"
...
def _init_cxl(config: CxlEngramStoreConfig):
...
def _allocate_bytes(key: int, size_bytes: int, config: CxlEngramStoreConfig) -> int:
...
class CxlEngramStore(EngramStore):
    """CXL-backed embedding store compatible with the EngramStore interface."""
    ...
    def __init__(
        ...
        config: Optional[CxlEngramStoreConfig] = None,
    ) -> None:
        super().__init__(embedding_dim, vocab_size, layer_id, dtype)
        self.config = CxlEngramStoreConfig.from_env(config)
        self.engram_layer_ids = list(engram_layer_ids)
        self.last_layer_id = (
            self.engram_layer_ids[-1] if self.engram_layer_ids else self.layer_id
        )
        self.storage_dtype = torch.float16
        self.ext = _init_cxl(self.config)
        self._ready_checked = False

        self.row_bytes = self.embedding_dim * 2  # float16
        total_bytes = self.vocab_size * self.row_bytes
        self.base_offset = _allocate_bytes(self.layer_id, total_bytes, self.config)

        self.buffer_num = 102400
        self.global_buffer_cpu = torch.empty(
            (self.buffer_num, self.embedding_dim),
            dtype=self.storage_dtype,
            device="cpu",
        )
        self.global_buffer_cuda: Optional[torch.Tensor] = None

        logger.info("Initialized CXL EngramStore for layer %d", self.layer_id)
    ...
    def get_one(self, index: int, layer_id: int, device: torch.device) -> torch.Tensor:
        ...
        out = torch.empty(
            (self.embedding_dim,), dtype=self.storage_dtype, device=device
        )
        self.ext.cxl_to_tensor_noflush(
            out, offset=int(self._offset_for_index(int(index)))
        )
    ...
    def get_many(
        ...
        if device.type == "cpu":
            out = self.global_buffer_cpu[:num_elements]
        else:
            if (
                self.global_buffer_cuda is None
                or self.global_buffer_cuda.device != device
            ):
                self.global_buffer_cuda = torch.empty(
                    (self.buffer_num, self.embedding_dim),
                    dtype=self.storage_dtype,
                    device=device,
                )
            out = self.global_buffer_cuda[:num_elements]

        offsets = indices * self.row_bytes + self.base_offset
        if num_elements >= 1:
            self.ext.cxl_to_tensor_noflush_parallel_contiguous_dst(
                out, int(self.row_bytes), offsets
            )
    ...
    def _set_ready_flag(self, value: int = 1) -> None:
    ...
    def _ensure_ready(self) -> None:
        ...
        while True:
            if self._read_ready_flag() == 1:
                logger.info("CXL Engram ready flag set")
