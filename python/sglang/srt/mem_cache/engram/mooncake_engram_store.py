...
"""Mooncake-backed Engram embedding store.

Uses MooncakeDistributedStore for distributed embedding storage.  Embedding
tables are sharded into fixed-size blocks and distributed across Mooncake
servers.  Compatible with the :class:`EngramStore` interface so it can be
used as a drop-in replacement for :class:`LocalEngramStore`.
"""
...
@dataclass
class MooncakeEngramStoreConfig:
    ...
    @staticmethod
    def from_env(
        base: Optional["MooncakeEngramStoreConfig"] = None,
    ) -> "MooncakeEngramStoreConfig":
        base = base or MooncakeEngramStoreConfig()
        return MooncakeEngramStoreConfig(
            local_hostname=os.getenv("MOONCAKE_LOCAL_HOSTNAME", base.local_hostname),
            metadata_server=os.getenv("MOONCAKE_METADATA_SERVER", base.metadata_server),
            ...
            vocab_block_size=_env_int(
                "MOONCAKE_VOCAB_BLOCK_SIZE", base.vocab_block_size
            ),
        )
...
def _create_mooncake_store(
    config: MooncakeEngramStoreConfig,
) -> MooncakeDistributedStore:
    if MooncakeDistributedStore is None:
        raise ImportError("Mooncake is not installed. Please install 'mooncake'.")
    logger.info("Creating Mooncake Distributed Store with config: %s", config)
...
def get_mooncake_store(
    ...
    if _GLOBAL_STORE is None:
        _GLOBAL_STORE = _create_mooncake_store(config)
...
class MooncakeEngramStore(EngramStore):
    ...
    def __init__(
        ...
        config: Optional[MooncakeEngramStoreConfig] = None,
    ) -> None:
        super().__init__(embedding_dim, vocab_size, layer_id, dtype)
        self.config = MooncakeEngramStoreConfig.from_env(config)
    ...
    def put_sharded(self, vocab_table: torch.Tensor) -> None:
        ...
        keys, values = [], []
        num_blocks = (
            self.vocab_size + self.vocab_block_size - 1
        ) // self.vocab_block_size
    ...
    def _sync_all_sharded(self) -> None:
        ...
        all_keys = [
            self._block_key(block_id, self.layer_id) for block_id in range(num_blocks)
        ]
        self._wait_key_data(all_keys, timeout_s=20.0)
    ...
    def get_one(self, index: int, layer_id: int, device: torch.device) -> torch.Tensor:
        ...
        if not data or len(data) < required:
            logger.warning(
                "get empty data from mooncake store for index %d layer_id %d",
                index,
                layer_id,
            )
            ...
            vec = np.frombuffer(
                data, dtype=np.float16, count=self.embedding_dim, offset=base * 2
            ).copy()
        return torch.from_numpy(vec).to(device=device, dtype=self.dtype)
    ...
    def get_many(
        ...
        return (
            ...
            .view(*indices.shape, self.embedding_dim)
        )
