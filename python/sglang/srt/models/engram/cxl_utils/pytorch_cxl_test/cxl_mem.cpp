...
bool cxl_init(const std::string &dev_path,
	...
	void *addr = ::mmap(nullptr, aligned_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, offset);
	::close(fd);
	if (addr == MAP_FAILED) {
	...
	if (register_cuda) {
		...
		if (cuda_check(cudaHostRegister(addr, aligned_size, cudaHostRegisterPortable), "cudaHostRegister")) {
...
bool cxl2dram_noflush_parallel(void **dram_dst_list,
					   ...
					   int num_threads) {
	// if (g_cxl.base == nullptr) {
	// 	std::cerr << "cxl2dram_noflush_parallel: invalid region\n";
	// 	return false;
	// }
	// if (count == 0) {
	// 	return true;
	// }
	// if (dram_dst_list == nullptr || bytes_list == nullptr || offset_list == nullptr) {
	// 	std::cerr << "cxl2dram_noflush_parallel: null list pointer\n";
	// 	return false;
	// }
	// for (std::size_t i = 0; i < count; ++i) {
	// 	if (offset_list[i] + bytes_list[i] > g_cxl.length) {
	// 		std::cerr << "cxl2dram_noflush_parallel: invalid range\n";
	// 		return false;
	// 	}
	// }
	// if (num_threads <= 0) {
	...
	for (std::int64_t i = 0; i < static_cast<std::int64_t>(count); ++i) {
		...
		const void *src = static_cast<const std::uint8_t *>(g_cxl.base) + offset_list[idx];
