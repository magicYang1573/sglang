...
def main() -> None:
    ...
    results.append(
        _check(
            "MooncakeEngramStore (import only)",
            lambda: __import__(
                "sglang.srt.mem_cache.engram.mooncake_engram_store",
                fromlist=["MooncakeEngramStore"],
            ),
        )
    )
    results.append(
        _check(
            "CxlEngramStore (import only)",
            lambda: __import__(
                "sglang.srt.mem_cache.engram.cxl_engram_store",
                fromlist=["CxlEngramStore"],
            ),
        )
    )
    results.append(
