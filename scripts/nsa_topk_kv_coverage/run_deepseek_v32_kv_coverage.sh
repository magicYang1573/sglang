...
echo "Generating fixed-length ShareGPT-derived dataset"
...
"${PYTHON_BIN}" scripts/nsa_topk_kv_coverage/run_mixed_length_requests.py \
  --base-url "${BASE_URL}" \
  --model "${MODEL_PATH}" \
  --dataset-jsonl "${run_dir}/dataset.jsonl" \
  --output-jsonl "${run_dir}/request_outputs.jsonl" \
  --concurrency 64 \
  2>&1 | tee "${run_dir}/run_requests.log"

echo "Analyzing decode-time KV coverage"
"${PYTHON_BIN}" scripts/nsa_topk_kv_coverage/analyze_decode_kv_coverage.py \
