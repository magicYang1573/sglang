...
__global__ void cxl2vram_copy_kernel(const std::uint8_t *base,
	...
	for (std::size_t i = static_cast<std::size_t>(threadIdx.x); i < bytes_per_segment; i += static_cast<std::size_t>(blockDim.x)) {
		out[i] = src[i];
	}
}
...
bool cxl_register_cuda_region(void *addr,
					  std::size_t length,
					  int gpu_id,
					  CxlRegion *region) {
	if (region == nullptr) {
	...
	if (!cuda_check(cudaHostRegister(addr, length, cudaHostRegisterPortable | cudaHostRegisterMapped), "cudaHostRegister")) {
		return false;
	}
	region->cuda_registered = true;
	if (!cuda_check(cudaHostGetDevicePointer(&region->device_ptr, addr, 0), "cudaHostGetDevicePointer")) {
...
bool cxl_unregister_cuda_region(CxlRegion *region) {
	if (region == nullptr || !region->cuda_registered || region->base == nullptr) {
		return true;
	}
...
bool vram2cxl_async(const void *device_src, std::size_t bytes, std::size_t offset) {
	...
	if (!cuda_check(cudaMemcpyAsync(dst, device_src, bytes, cudaMemcpyDeviceToHost, stream.stream()), "D2H")) {
		return false;
	}
...
bool cxl2vram_noflush_parallel_contiguous_dst(void *device_dst,
	...
	if (!g_cxl.cuda_registered || g_cxl.device_ptr == nullptr) {
		std::cerr << "cxl2vram_noflush_parallel_contiguous_dst: CXL region not CUDA-mapped\n";
		return false;
	}
	...
	if (!cuda_check(cudaMemcpyAsync(g_offsets_dev, offset_list, offsets_bytes, cudaMemcpyHostToDevice, stream.stream()), "cudaMemcpyAsync offsets")) {
		return false;
	}
	...
	cxl2vram_copy_kernel<<<grid, threads, 0, stream.stream()>>>(
		...
		static_cast<std::uint8_t *>(device_dst),
		g_offsets_dev,
		bytes_per_segment);
	if (!cuda_check(cudaGetLastError(), "cxl2vram_copy_kernel launch")) {
		return false;
	}
	if (!cuda_check(cudaStreamSynchronize(stream.stream()), "cudaStreamSynchronize")) {
		return false;
	}
	return true;
}
