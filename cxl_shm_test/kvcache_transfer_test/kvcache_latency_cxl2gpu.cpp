// Benchmark CXL-backed KV cache copies from DAX to GPU memory.
// Modeled after kvcache_latency_cxl2dram.cpp but measures host->device copies.
...
Options parse_args(int argc, char** argv) {
	...
	for (int i = 1; i < argc; ++i) {
		...
		if (arg == "--dax" || arg == "--dev") {
			...
			std::cout << "Usage: ./kvcache_latency_cxl2gpu [options]\n"
...
void copy_to_gpu_contiguous(const Options& opt, const Value* src, Value* device_dst, cudaStream_t stream) {
	const size_t token_stride = opt.layers_total * opt.heads * opt.head_dim;
	const size_t layer_stride = opt.heads * opt.head_dim;
	const size_t bytes_per_layer = layer_stride * sizeof(Value);

	// Copy per (layer, token) block to keep addressing logic identical to CPU test.
	...
	for (size_t l = 0; l < opt.layers; ++l) {
		for (size_t t = 0; t < opt.tokens; ++t) {
			...
			clflush_range(src_ptr, bytes_per_layer);
			check_cuda(cudaMemcpyAsync(dst_ptr, src_ptr, bytes_per_layer, cudaMemcpyHostToDevice, stream), "cudaMemcpyAsync contiguous");
		}
	}
}

void copy_to_gpu_random(const Options& opt, const Value* src, Value* device_dst, cudaStream_t stream, const std::vector<size_t>& token_indices_per_layer) {
	const size_t token_stride = opt.layers_total * opt.heads * opt.head_dim;
	const size_t layer_stride = opt.heads * opt.head_dim;
	const size_t bytes_per_layer = layer_stride * sizeof(Value);
	...
	for (size_t l = 0; l < opt.layers; ++l) {
		for (size_t t = 0; t < opt.tokens; ++t) {
			...
			Value* dst_ptr = device_dst + ((l * opt.tokens + t) * layer_stride);
			clflush_range(src_ptr, bytes_per_layer);
			check_cuda(cudaMemcpyAsync(dst_ptr, src_ptr, bytes_per_layer, cudaMemcpyHostToDevice, stream), "cudaMemcpyAsync random");
		}
	}
}

Stats benchmark_contiguous(const Options& opt, const Value* src, Value* device_dst, cudaStream_t stream) {
	std::vector<double> samples;
	samples.reserve(opt.iterations);

	for (size_t i = 0; i < opt.warmup + opt.iterations; ++i) {
		auto t0 = std::chrono::steady_clock::now();
		copy_to_gpu_contiguous(opt, src, device_dst, stream);
		check_cuda(cudaStreamSynchronize(stream), "stream sync contiguous");
...
Stats benchmark_random(const Options& opt, const Value* src, Value* device_dst, cudaStream_t stream) {
	std::vector<double> samples;
	samples.reserve(opt.iterations);
	...
	for (size_t i = 0; i < opt.warmup + opt.iterations; ++i) {
		auto t0 = std::chrono::steady_clock::now();
		copy_to_gpu_random(opt, src, device_dst, stream, token_indices);
		check_cuda(cudaStreamSynchronize(stream), "stream sync random");
		auto t1 = std::chrono::steady_clock::now();
		double ns = std::chrono::duration<double, std::nano>(t1 - t0).count();
		if (i >= opt.warmup) samples.push_back(ns);
	}
...
int main(int argc, char** argv) {
	try {
		...
		if (CudaCheck(cudaHostRegister(reinterpret_cast<uint8_t*>(map.ptr), bytes_needed, cudaHostRegisterPortable), "cudaHostRegister")) {
			registered = true;
		}
		...
		Stats st_contig = benchmark_contiguous(opt, map.ptr, device_dst, stream);
		Stats st_random = benchmark_random(opt, map.ptr, device_dst, stream);
